...
class ReviewSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        ...
        event_id = self.context["request"].parser_context["kwargs"]["event_id"]
        if Review.objects.filter(author=author, event=event_id).exists():
            raise serializers.ValidationError("Вы уже оставили отзыв на данное событие")
        event = get_object_or_404(Event, id=event_id)
        if not EventParticipant.objects.filter(event=event_id, user=author).exists():
